...
export default function ContactsPage() {
  const [formState, setFormState] = useState({
    nome: '',
    numero: '', // Alterado de 'email' para 'numero'
    mensagem: '',
  });
  ...
  const handleChange = (e) => {
    ...
    // Limita a mensagem a 155 palavras
    if (name === 'mensagem') {
  ...
  const validate = () => {
    let newErrors = { nome: '', numero: '', mensagem: '' }; // Alterado de 'email' para 'numero'
    let isValid = true;
    ...
    if (!formState.numero.trim()) {
      ...
      // Regex para validar números de 9 dígitos começando com 82, 83, 84, 85, 86, 87, 88
      const phoneRegex = /^(82|83|84|85|86|87|88)\d{7}$/;
      if (!phoneRegex.test(formState.numero.trim())) {
        newErrors.numero = 'O número deve ter 9 dígitos e começar com 82, 83, 84, 85, 86, 87 ou 88.';
        isValid = false;
      }
    }
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-16 items-start mt-8">
          ...
          <div className="p-8 rounded-2xl shadow-xl border border-gray-100 mb-6 bg-white">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <input
                  type="tel" // Alterado para 'tel' para melhor usabilidade em celulares
                  name="numero" // Alterado para 'numero'
                  placeholder="Seu Número de Telefone *"
                  value={formState.numero}
                  onChange={handleChange}
                  className={`w-full p-3 rounded-lg border bg-gray-50 text-gray-800 placeholder-gray-500 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:border-transparent ${errors.numero ? 'border-red-500' : 'border-gray-200'}`}
                />
