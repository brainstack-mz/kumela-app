...
export default function ProductCard({ product, onPreview, onAddToCart }: ProductCardProps) {
  const finalPrice = product.discount 
    ? product.price - product.price * (product.discount / 100)
    : product.price;

  const handleAddToCart = (e: React.MouseEvent) => {
    e.stopPropagation(); // Impede o clique de abrir a pré-visualização
    ...
    >
      <div className="relative w-full h-48 sm:h-56">
        ...
        <button
          ...
          className="absolute bottom-2 right-2 bg-white bg-opacity-80 rounded-full p-3 flex items-center justify-center shadow-lg hover:bg-opacity-100 transition"
